...
export class Rule34Command extends BaseCommand {
    ...
    async action(): Promise<string | void> {
        ...
        const resultImage = randomItem(images);
    ...
    async awaitUserReactionAndAppReacting(message: Message): Promise<unknown> {
        const appReacting = reactMultiple(message, [
            Rule34Command.EMOJI_NEXT,
            Rule34Command.EMOJI_DELETE,
        ]);
        const userReaction = await awaitEmojiReaction(
            message,
